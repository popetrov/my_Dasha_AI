...
const audioGatewayProtocol = __importStar(require("./grpc-api/node/audio-gateway-protocol"));
const audioStreamProtocol = __importStar(require("./grpc-api/node/audio-stream-protocol"));
...
class AudioChannel extends eventemitter3_1.default {
    /** @internal */ constructor(clientAccount, options) {
        ...
        this._handleTerminate = (roomId, reason, details) => {
            if (reason === audioGatewayProtocol.TerminationReason.DISCONNECTED) {
                const error = new errors.AudioRoomDisconnectedError(roomId, details);
                this.emit("error", error);
            }
            if (reason === audioGatewayProtocol.TerminationReason.FORBIDDEN) {
                const error = new errors.AudioRoomForbiddenError(roomId, details);
                this.emit("error", error);
            }
            if (reason === audioGatewayProtocol.TerminationReason.NOT_FOUND) {
    ...
    async _init(clientAccount, options) {
        ...
        this._gatewayClient = new audioGatewayProtocol.AudioGatewayProtocolClient(clientAccount, this._log);
        this._audioStreamClient = new audioStreamProtocol.AudioStreamProtocolClient(clientAccount, this._log);
        ...
        this._backendAudioStream.writeInfo({
            roomId: this._roomId,
            encoding: audioStreamProtocol.AudioEncoding.LINEAR16,
            sampleRate: 8000,
        });
